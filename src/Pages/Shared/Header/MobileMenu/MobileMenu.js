...
const MobileMenu = (props) => {
    ...
    return (
        <div className={`absolute top-0 left-0 ${props.active ? 'w-0' : 'w-10/12'} h-screen bg-blue-300 text-white transition-all ease-in-out duration-300`}>
            {!props.active && <button className=" text-gray-800 font-bold absolute right-5 top-5" onClick={() => props.toggle()}>&#x2715;</button>}
            <div className=" h-screen w-full flex justify-center items-center">
                <div className={`${props.active ? 'invisible' : 'visible'} w-full text-center overflow-hidden transition-all ease-in-out delay-300`}>
